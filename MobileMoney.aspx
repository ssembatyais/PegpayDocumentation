...
<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
    <div class="row">
        <div class="col-md-4 mt-5" style="background-color:#ebeef5; margin-top:20px; height:90vh;">
         <ul>
        <li class="bd-links-group py-2 list-unstyled">
       <h2> <strong class="bd-links-heading d-flex w-100 align-items-center fw-semibold rounded active">
          Get Started
        </strong></h2>
       
        <ul class="list-unstyled fw-normal pb-2 small">

                  <li class="sidebarList" > <asp:LinkButton ID="LinkButton0" runat="server" class="bd-links-link d-inline-block rounded" OnClick="LinkButton0_Click" >Introduction</asp:LinkButton></li>
          
            <li class="sidebarList" > <asp:LinkButton ID="LinkButton1" runat="server" class="bd-links-link d-inline-block rounded" OnClick="LinkButton1_Click" >Integration Steps And Sandbox</asp:LinkButton></li>

             <li class="sidebarList" > <asp:LinkButton ID="LinkButton2" runat="server" class="bd-links-link d-inline-block rounded" OnClick="LinkButton2_Click" >Api Functions</asp:LinkButton></li>

             <li class="sidebarList" > <asp:LinkButton ID="LinkButton3" runat="server" class="bd-links-link d-inline-block rounded" OnClick="LinkButton3_Click" >PayIn</asp:LinkButton></li>

             <li class="sidebarList" > <asp:LinkButton ID="LinkButton4" runat="server" class="bd-links-link d-inline-block rounded" OnClick="LinkButton4_Click" >Payout </asp:LinkButton></li>
               <li class="sidebarList" > <asp:LinkButton ID="LinkButton5" runat="server" class="bd-links-link d-inline-block rounded" OnClick="LinkButton5_Click" >Request Parameters</asp:LinkButton></li>

             <li class="sidebarList" > <asp:LinkButton ID="LinkButton6" runat="server" class="bd-links-link d-inline-block rounded" OnClick="LinkButton6_Click" >Get Transaction Details</asp:LinkButton></li>
         <li class="sidebarList" > <asp:LinkButton ID="LinkButton7" runat="server" class="bd-links-link d-inline-block rounded" OnClick="LinkButton7_Click" >Get Account Balance</asp:LinkButton></li>
          <li class="sidebarList" > <asp:LinkButton ID="LinkButton8" class="bd-links-link d-inline-block rounded"  runat="server" OnClick="LinkButton8_Click" >Digital Signatures</asp:LinkButton></li>
               <li class="sidebarList" > <asp:LinkButton ID="LinkButton9" class="bd-links-link d-inline-block rounded"  runat="server" OnClick="LinkButton9_Click" >Security Requirements</asp:LinkButton></li>
               <li class="sidebarList" > <asp:LinkButton ID="LinkButton10" class="bd-links-link d-inline-block rounded"  runat="server" OnClick="LinkButton10_Click" >Status Codes and Description</asp:LinkButton></li>
     ...
     <p> Pegasus Technologies Ltd is a Payments’ Aggregator/Integrator company with existing relationships with Banks and Mobile Money Service Providers through which customers can settle their payments. Pegasus Technologies also has existing relationships with a number of
    ...
    <h3>Purpose.</h3> 
    <p>Pegasus Technologies provides an interface to all partner telecom and bank e-wallet systems
...
<p>This technical document is therefore intended for system developers who intend to carry out
integration with the PegPay Payments and Collections platform. It describes the message types
that the PegPay system expects and the responses returned from the system.</p>               
<p>Pegasus Technologies has extensive experience and expertise in third-party system integrations
and will provide technical guidance during the integration process if the client requests it.</p>
 </div>
  </section>
             </asp:View> 
...
platform offers a robust sandbox that simplifies the testing and certification process. </p>

     <p> The sandbox allows you to send API requests and view the responses in real time. The sandbox also comes equipped with a test simulator that allows you to create an assortment of test cases, which will
ensure you are able to test against a wide variety of scenarios.</p>   
...
be used when verifying the digital signatures for all requests made to the system. Pegasus will need the following data from the vendor to create a sandbox:</p>
<p>
...
<P>This functionality is used by the agent to get the details of a phone number/Bank Account of an
...
<p>1. Whitelist</p>
<p>This extra parameter is useful for instructing the PegPay Payments and Collections API to
only complete a transaction if the number is whitelisted. Possible whitelist check options
are FROMACCOUNT, TOACCOUNT, BOTH
"AddendumData":"whitelist:FROMACCOUNT"</p>
...
in real time.
 ...
 <p>All transactions posted to the PegPay Payments and Collections Platform have to be digitally signed. The purpose of the digital signature is to ensure that the transactions are received from agents who claim to be who they are. The order of the concatenation of the data should be maintained as given below. </p>
   <strong>
                     ...
                     PostTransaction: dataToSign (CustomerRef + CustomerName + FromTelecom +ToTelecom + VendorTranId + VendorCode + Password + PaymentDate + TranType + PaymentCode + TranAmount + FromAccount + ToAccount)
                </strong>
     <strong>
                  ValidatePhoneNumber: dataToSign (PhoneNumber + VendorCode + Password)
             </strong>
...
data from authorized agents is processed </li>
                     <li>
                         IP Whitelisting: On production, all data processed by PegPay Payments and Collections
...
been whitelisted. Data from other unknown IPs will be rejected by the billingin
                 ...
                 <p>Vendors who wish to view and download statements of transactions done using the PegPay
Payments and Collections Platform will be given access to a Web Portal from which they can
login, view, download and do reconciliation for all transactions done  </p>
             </div>
         </section>
                 </asp:View> 
